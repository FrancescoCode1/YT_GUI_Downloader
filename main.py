...
def downloader():
    ...
    layout = [
        ...
        [sg.Text("Format                 "), sg.Radio("wav", key="-WAV-", group_id="format"), sg.Radio("mp3", key="-MP3-", group_id="format"), sg.Radio("mp4", key="-MP4-", group_id="format")],
    ...
    while True:
        ...
        if event == "-CONVERT-":
            ...
            if mp3:
                ...
                for i in music_list:
                    os.system(f"yt-dlp -f bv*+ba {i} -o \"\download\%(title)s.%(ext)s\" ")
                sg.popup(f"DONE!")
